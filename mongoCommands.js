...
//skip first 2 book with highest rating
db.books.find({}).sort({rating:-1}).skip(2).pretty();

//combine include + exclude
db.books.find({},{ _id: 0 , casts: 0 }).pretty();

//combine  operator + projection + sorting
db.books.find({ rating:{ $gt:8 } },{ _id: 0, casts: 0 }).sort({ rating: -1 } ).pretty();

//sort + projection - id(exclude), name,rating(include), sort(desc), limit-2
db.books.find({},{ _id: 0, name: 1, rating: 1}).sort({ rating: -1}).limit(2).pretty();

//sort by rating in desc and include summary and trailer, skip- 2
db.books.find({},{ _id: 0, summary: 1, trailer: 1 }).sort({ rating: -1}).skip(2).pretty();

//find only one doc where rating = 8.8
db.books.find({ rating: {$eq:8.8}}).pretty();

//Aggregations
//Select sum(quantity)  from the orders where status = "urgent"
...
//stage -2
//$match, $group, $sum - aggregations methods, operators
db.orders.aggregate([{ $match: { status: "urgent" }}, { $group: { _id: "$productName", totalUrgentQuantity: {$sum: "$quantity"}}}]);


//Tasks
//1. update the language for all docs
// db.books.updateMany({ language:"English" }).pretty();
db.books.updateMany({},{ $set: {language:"English"} }).pretty();

//2. Alai Payuthey update rating to 9
// db.books.updateOne({ rating:9 },{ name: "Alai Payuthey"}).pretty();
db.books.updateOne({name:"Alai Payuthey"}, { $set: {rating: 9} }); 

//To update  language as english in a specific movie
db.books.updateOne({name:"Alai Payuthey"}, { $set: {language:"English"} }); 

//3. Delete all books with rating > 8.6
// db.books.deleteMany({},{ rating: {$gt: 8.6}}).pretty();
db.books.deleteMany({ rating: {$gt: 8.6}}).pretty();